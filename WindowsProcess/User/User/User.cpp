...
class ProcessHandler {
    const LPCSTR killer_path = "..\\..\\Killer\\x64\\Debug\\Killer.exe";
public:
...
int main() {
    ...
    SetEnvironmentVariableA("PROC_TO_KILL", "Telegram,WarThunder");
    auto currentActiveProcesses = handler.getActiveProcesses();
    for (auto x : currentActiveProcesses) {
        if (x.NAME == L"Telegram.exe" || x.NAME == L"WarThunder.exe") {
